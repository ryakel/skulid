...
type Account struct {
	...
	CreatedAt            time.Time
	// Set when Google permanently rejected this account's refresh token.
	// The account stays in the DB (rules and blocks keep their config) but
	// every sync is a no-op until the owner reconnects it.
	NeedsReauth      bool
	ReauthReason     string
